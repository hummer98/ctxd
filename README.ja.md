...
| claude-opus-4-7 | 0.2.0 | 3 | 15 | 100.0% | 6/6 | 6/6 | 3/3 | SKILL.md Postcondition 章を T010 完成版 (`--expect` DSL + 失敗時も Result.Data 保持) に書き換え (T023) |
...
SKILL を導入することでトークン / tool_use 数 / wall time がどう変わったか。
上の adherence は「`ctxd` を呼んだか」、こちらは「呼ぶまでに何を消費したか」を測ります。
数値は同じ `evals/run.sh` (`EVAL_N=3` × 5 scenarios) で取った 1 trial あたりの平均です。

| model | plugin version | avg_tool_uses | avg_input_tokens | avg_output_tokens | avg_wall_ms | notes |
|---|---|---:|---:|---:|---:|---|
| claude-opus-4-7 | ≤ 0.2.0 | – | – | – | – | T024 以前の run では session JSONL が gitignore で pruned されており、効率指標を遡及で復元できないため空欄。 |

`avg_input_tokens` は trial 内の `assistant` メッセージの `input_tokens`
(fresh) を全合算した値です (cache write / read は run 別の `summary.md` に
内訳が出ます)。`avg_wall_ms` は秒精度 (`meta.json` の `wall_seconds` ×
1000)。Stage 1 baseline は本コミット後に最初の `evals/run.sh` を回した時点
で採取されます。それまでは「過去 run から復元できなかった」ことを上の行で
記録しています。
